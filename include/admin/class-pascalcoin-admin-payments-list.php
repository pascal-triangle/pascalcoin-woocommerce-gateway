...
class Pascalcoin_Admin_Payments_List extends WP_List_Table {
    ...
    function extra_tablenav($which) {
        if ($which == "top") {
            ...
            $balance = Pascalcoin_Gateway::admin_balance_info();
            $balance_info = <<<HTML
...
HTML;
            echo <<<HTML
            <div class="wrap">
                <h1 class="wp-heading-inline">PascalCoin Payments</h1>
                $balance_info
                <hr class="wp-header-end">
                <ul class="subsubsub">
                    <li>
                        <a href="?page=pascalcoin_gateway_payments&type=all" {$tab_info['all']['active']}>
                            All <span class="count">({$tab_info['all']['count']})</span>
                        </a> |
                    </li>
                    <li style="display:none">
                        <a href="?page=pascalcoin_gateway_payments&type=pending" {$tab_info['pending']['active']}>
                            Pending <span class="count">({$tab_info['pending']['count']})</span>
                        </a> |
                    </li>
                    <li>
                        <a href="?page=pascalcoin_gateway_payments&type=paid" {$tab_info['paid']['active']}>
                            Received <span class="count">({$tab_info['paid']['count']})</span>
                        </a> |
                    </li>
                    <li>
                        <a href="?page=pascalcoin_gateway_payments&type=confirmed" {$tab_info['confirmed']['active']}>
                            Confirmed <span class="count">({$tab_info['confirmed']['count']})</span>
                        </a> |
                    </li>
                    <li>
                        <a href="?page=pascalcoin_gateway_payments&type=expired" {$tab_info['expired']['active']}>
                            Expired <span class="count">({$tab_info['expired']['count']})</span>
                        </a>
                    </li>
                </ul>
                <form id="pascalcoin-payments-filter" method="get" style="display:none">
                    <p class="search-box">
                ...
                <h2 class="screen-reader-text">PascalCoin Payments List</h2>
                <style>
...
HTML;
    ...
    public function column_default($item, $column_name) {
        ...
        case 'col_payment_id':
            ...
            break;
        case 'col_txid':
            $url = PASCALCOIN_GATEWAY_EXPLORER_URL.'/findoperation.php?ophash='.$item->txid;
        ...
        case 'col_height':
            ...
            break;
        case 'col_amount':
            echo Pascalcoin_Gateway::format_pascalcoin($item->amount).' PASC';
            break;
        }
    }
    ...
    public function no_items() {
        esc_html_e('No Pascalcoin payments found', 'pascalcoin_gateway');
    }
